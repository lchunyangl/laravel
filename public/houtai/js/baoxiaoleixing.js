$(function () {
    ...
    $('.right .title .xz').click(function () {
        ...
        $('.caozuo ul .baocun').click(function () {
            var bh = $('#bh').val();
            var baoxiaolx_name = $('#baoxiaolx_name').val();
            var zjm = $('#zjm').val();
            if (bh == "" || baoxiaolx_name == "" || zjm == "") {
                alert("加星号的为必填项");
                return false;
            } else {
                $.ajax({
                    url: '/baoxiaolx',
                    type: 'post',
                    data: {bh: bh, baoxiaolx_name: baoxiaolx_name, zjm: zjm},
                    dataType: 'json',
            ...
            $('#warning .qd').click(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.error == 0) {
                            $('.td_input input:checked').each(function () {
                                var baoxiaolx_id = $(this).val();
                                $('#li' + baoxiaolx_id).remove();
                            });
