$(function () {
    ...
    $('.right .title .xz').click(function () {
        ...
        $('.caozuo ul .baocun').click(function () {
            var bh = $('#bh').val();
            var renyuan_name = $('#renyuan_name').val();
            var bumen_id = $('#bumen_id').val();
            var zjm = $('#zjm').val();
            var type = $('#type').val();
            var status = $('#status').val();
            if (bh == "" || renyuan_name == "" || bumen_id == "" || status == "" || type == "") {
                alert("加星号的为必填项")
            } else {
                $.ajax({
                    url: '/renyuan',
...
function query() {
    var str = document.getElementById("renyuan_name").value.trim();
    if (str == "") return;
    var arrRslt = makePy(str);
    //循环将值到下拉框
    //  var option = null;
    document.getElementById("zjm").value = ""; //清空下拉框
    for (var j = 0; j < arrRslt.length; j++) {
        var obj = document.getElementById("zjm");
        obj.value = arrRslt[j];
    }
}
